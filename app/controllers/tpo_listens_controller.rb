...
class TpoListensController < ApplicationController
  def index
    ...
    @tpo_questions = TpoQuestion.includes(:tpo_type).joins(tpo_type: :tpo_group).where(tpo_types: {name: ['Conversion', 'Lecture']}).order("tpo_types.name, sequence_number").page(params[:page])
  end
